/* 2-3-base ANCHOR-RUSH attacker v33: rush gate open from the FIRST base
   (ANCHOR_ROUTE_START_BASES_B=0) - earliest possible aggression. */
/* species2b ANCHOR-ROUTE champion gen 49 (vsP2 0.707, vsG15off 0.673; ENABLE_ANCHOR_ROUTE_ATTACKS=1). All 105 params baked. */
...
#define BUILD_RESERVE 219
#endif
#ifndef EXPANSION_MOVE_RESERVE
#define EXPANSION_MOVE_RESERVE 6
#endif
...
#define HP_RATIO_TRAIN_PRIORITY_KEEP_SOLVENT 0
#endif
...
#define OPENING_NEUTRAL_FIRST_MAX_TURN 45
#endif
#ifndef OPENING_NEUTRAL_FIRST_TARGET_PERCENT
#define OPENING_NEUTRAL_FIRST_TARGET_PERCENT 1
#endif
#ifndef OPENING_NEUTRAL_FIRST_MIN_TARGETS
#define OPENING_NEUTRAL_FIRST_MIN_TARGETS 2
#endif
#ifndef OPENING_NEUTRAL_FIRST_MAX_TARGETS
#define OPENING_NEUTRAL_FIRST_MAX_TARGETS 3
#endif
...
#define OPENING_RELAX_HQ_KEEP_UNTIL_TURN 62
#endif
#ifndef OPENING_DELAY_HQ_UPGRADE_UNTIL_TURN
#define OPENING_DELAY_HQ_UPGRADE_UNTIL_TURN 25
#endif
...
#define OPENING_FORCE_ONE_NEUTRAL_PER_TURN_UNTIL 35
#endif
#ifndef OPENING_FORCE_TRAIN_NEUTRAL_UNTIL
#define OPENING_FORCE_TRAIN_NEUTRAL_UNTIL 35
#endif
...
#define THIN_OPENING_MAX_TURN 68
#endif
#ifndef THIN_OPENING_RUSH_SAFETY_TURNS
#define THIN_OPENING_RUSH_SAFETY_TURNS 3
#endif
...
#define OPENING_NEUTRAL_ABORT_ENEMY_MASS_ETA 14
#endif
#ifndef OPENING_NEUTRAL_ABORT_ENEMY_MASS
#define OPENING_NEUTRAL_ABORT_ENEMY_MASS 5
#endif
...
#define ENDGAME_ATTACK_TURNS 11
#endif
...
#define FORWARD_STAGING_MAX_PER_SOURCE 1
#endif
#ifndef FORWARD_STAGING_EXTRA_CAP
#define FORWARD_STAGING_EXTRA_CAP 7
#endif
...
#define STACK_TARGET_DANGER_COUNT 5
#endif
...
#define STACK_CLEANUP_MAX_TARGET_ETA 9
#endif
...
#define STACK_CLEANUP_EXTRA_MARGIN 0
#endif
...
#define ENABLE_RETREAT_FOLLOWUP_BLOCK 0
#endif
#ifndef RETREAT_SIM_DAYS
#define RETREAT_SIM_DAYS 54
#endif
...
#define ANCHOR_ROUTE_MAX_STACK 14
#endif
#ifndef ANCHOR_ROUTE_LEAVE_AT_ANCHOR
#define ANCHOR_ROUTE_LEAVE_AT_ANCHOR 1
#endif
...
#define ANCHOR_ROUTE_MIN_ATTACKERS 8
#endif
...
#define ANCHOR_ROUTE_LEAVE_ON_CAPTURE 3
#endif
...
#define ANCHOR_ROUTE_STAGE_WITHOUT_READY_TARGET 1
#endif
...
#define HQ_SAVE_LOOKAHEAD_TURNS 4
#endif
...
#define MAX_CAPTURE_SIM_DAYS 76
#endif
#ifndef HQ_SIM_MAX_REINFORCE_DAYS
#define HQ_SIM_MAX_REINFORCE_DAYS 112
#endif
...
#define LATE_TIEBREAK_SAVE_LOOKAHEAD_TURNS 31
#endif
...
#define CASH_DUMP_TRAIN_START_TURN 50
#endif
#ifndef CASH_DUMP_MIN_GOLD
#define CASH_DUMP_MIN_GOLD 600
#endif
...
#define ENABLE_ANCHOR_RUSH 1
#endif
static int g_anchor_rush_active = 0;
static int g_win78_first_hq_wave_launched = 0;
static int g_win78_hq9_mode_started = 0;
/* Regions currently occupied/targeted by a launched 9-stack.  The first
   launch starts at HQ with 9, but after the stack destroys a base it should
   roll forward from the conquered region with all surviving soldiers instead
   of waiting for a new HQ packet. */
static int g_win78_chain_region_turn[512];
static int win78_chain_region_active(int r);
...
#endif
#if ENABLE_WINNER78_ATTACKER_PLAN
      && !win78_chain_region_active(w->region)
#endif
      )
    return 0;
...
#define OWNED_BASE_REINFORCE_SOURCE_RADIUS 9
#endif
#ifndef OWNED_BASE_MAX_EMERGENCY_PULL
#define OWNED_BASE_MAX_EMERGENCY_PULL 18
#endif
...
#define HQ_SIM_DEFENSE_THREAT_RADIUS 2
#endif
#ifndef HQ_SIM_DEFENSE_DAYS
#define HQ_SIM_DEFENSE_DAYS 24
#endif
#ifndef HQ_SIM_DEFENSE_MAX_RECALL
#define HQ_SIM_DEFENSE_MAX_RECALL 12
#endif
#ifndef HQ_SIM_DEFENSE_TRAIN_CAP
#define HQ_SIM_DEFENSE_TRAIN_CAP 3
#endif
...
static int collect_enemy_hq_attack_schedule(const GameState *S, const GameMap *M,
  ...
  for (int i = 0; i < S->warriors.len && n < maxn; ++i) {
    ...
    if (w->region == hq) {
      ...
      int h = path_hops_between(P, w->region, hq);
      if (h >= 0 && h <= HQ_SIM_DEFENSE_THREAT_RADIUS) eta = h; /* staged: worst case */
    }
...
static int issue_hq_exact_sim_defense(Actions *a, const GameState *S,
  ...
  if (hq_holds_scheduled_combat(S, M, P, a, NULL, NULL, 0, 0, 0)) return 0;
...
#define FALLBACK_MAX_PULL 8
#endif
#ifndef FALLBACK_TRAIN_CAP
#define FALLBACK_TRAIN_CAP 2
#endif
...
#define ANCHOR_RUSH_WAVE 9
#endif
#ifndef ANCHOR_RUSH_LEAVE_HOME
#define ANCHOR_RUSH_LEAVE_HOME 1
#endif

/* Winner-1-78 style attacker gate.
   The observed winner expands first, upgrades HQ to level 2, then sends a
   9-warrior wave from a forward anchor.  The ordinary map-size buckets below
   require 4/5/6 owned BASEs before the rush engine starts; on the 1-78 map
   (N=79) this chooses 6, matching the log timing before HQ2 and the first
   9-stack. */
#ifndef ENABLE_WINNER78_ATTACKER_PLAN
#define ENABLE_WINNER78_ATTACKER_PLAN 1
#endif
#ifndef WIN78_SMALL_N_MAX
#define WIN78_SMALL_N_MAX 63
#endif
#ifndef WIN78_MED_N_MAX
#define WIN78_MED_N_MAX 89
#endif
#ifndef WIN78_BASES_SMALL
#define WIN78_BASES_SMALL 4
#endif
#ifndef WIN78_BASES_MED
#define WIN78_BASES_MED 5
#endif
#ifndef WIN78_BASES_LARGE
#define WIN78_BASES_LARGE 6
#endif
#ifndef WIN78_HQ_MIN_LEVEL
#define WIN78_HQ_MIN_LEVEL 2
#endif

static int win78_required_bases_for_map(const GameMap *M) {
  if (M->N <= WIN78_SMALL_N_MAX) return WIN78_BASES_SMALL;
  if (M->N <= WIN78_MED_N_MAX) return WIN78_BASES_MED;
  return WIN78_BASES_LARGE;
}

static int win78_ready_for_anchor_rush(const GameState *S, const GameMap *M) {
#if ENABLE_WINNER78_ATTACKER_PLAN
  if (anchor_route_my_base_count(S, M) < win78_required_bases_for_map(M)) return 0;
  if (my_hq_level(S, M) < WIN78_HQ_MIN_LEVEL) return 0;
  return 1;
#else
  (void)S; (void)M;
  return 1;
...
static int win78_hq2_gate_should_run(const GameState *S, const GameMap *M) {
#if ENABLE_WINNER78_ATTACKER_PLAN
  if (anchor_route_my_base_count(S, M) < win78_required_bases_for_map(M)) return 0;
  return my_hq_level(S, M) < WIN78_HQ_MIN_LEVEL;
#else
  (void)S; (void)M;
  return 0;
#endif
...
static int issue_win78_hq2_priority(Actions *a, const GameState *S,
                                    const GameMap *M, const Paths *P,
                                    int *budget) {
#if ENABLE_WINNER78_ATTACKER_PLAN
  if (!win78_hq2_gate_should_run(S, M)) return 0;

  /* First bring a legal worker home if the HQ is empty, then upgrade as soon
     as affordable.  If the upgrade is not affordable yet, return 1 to reserve
     gold instead of drifting into extra base upgrades/training before HQ2. */
  int moved = ensure_hq_upgrade_worker(a, S, M, P, budget);
  if (issue_hq_upgrade_if_affordable(a, S, M, budget)) return 1;
  return moved ? 1 : 1;
#else
  (void)a; (void)S; (void)M; (void)P; (void)budget;
  return 0;
...
#ifndef ANCHOR_RUSH_HQ_WHEN_BASES_UNGUARDED
#define ANCHOR_RUSH_HQ_WHEN_BASES_UNGUARDED 1
#endif

static int anchor_rush_enemy_base_count(const GameState *S, const GameMap *M) {
  int n = 0;
  for (int i = 0; i < S->buildings.len; ++i) {
    const Building *b = &S->buildings.data[i];
    if (b->side == opposite(M->my_side) && b->type == BTYPE_BASE) ++n;
  }
  return n;
}

static int anchor_rush_enemy_base_with_warrior_count(const GameState *S,
                                                     const GameMap *M) {
  int guarded = 0;
  for (int i = 0; i < S->buildings.len; ++i) {
    const Building *b = &S->buildings.data[i];
    if (b->side != opposite(M->my_side) || b->type != BTYPE_BASE) continue;
    if (enemy_warrior_count_at(S, M, b->region) > 0) ++guarded;
  }
  return guarded;
}

static int anchor_rush_should_hit_hq_now(const GameState *S, const GameMap *M) {
  /* 1-78 rolling rule, tightened: do not switch to the enemy HQ just because
     enemy BASEs are empty/unguarded.  Keep cycling enemy BASEs until the
     opponent has zero BASE buildings left. */
  return anchor_rush_enemy_base_count(S, M) <= 0;
}

static int anchor_rush_path_hits_region_before_target(const Paths *P,
                                                      int src, int target,
                                                      int forbidden) {
  if (src < 0 || target < 0 || forbidden < 0) return 0;
  if (src == forbidden) return 1;
  int cur = src;
  for (int steps = 0; steps < 512 && cur != target; ++steps) {
    int nx = P->nxt[cur][target];
    if (nx < 0) return 0;
    cur = nx;
    if (cur == forbidden && cur != target) return 1;
  }
  return 0;
}


static int anchor_rush_region_is_enemy_objective(const GameState *S,
                                                 const GameMap *M,
                                                 int r) {
  const Building *b = find_building_const(S, r);
  if (b != NULL && b->side == opposite(M->my_side)) return 1;
  if (region_has_enemy_warrior(S, M, r)) return 1;
  return 0;
}

static int anchor_rush_region_requires_full_packet(const GameState *S,
                                                   const GameMap *M,
                                                   int r) {
  if (anchor_rush_region_is_enemy_objective(S, M, r)) return 1;
  if (r == M->my_hq) return 0;
  const Building *b = find_building_const(S, r);
  if (b != NULL && b->side == M->my_side) return 0;
  if (is_stronghold(M, r)) return 1;  /* empty/opponent stronghold: no solo claim/drip */
  return 0;
}

static int anchor_rush_is_new_capture_upgrade(const GameState *S,
                                              const GameMap *M,
                                              int r) {
  (void)M;
  return find_building_const(S, r) == NULL;
}

#ifndef WIN78_CHAIN_REGION_TTL
#define WIN78_CHAIN_REGION_TTL 80
#endif

static void win78_note_chain_region(int r) {
#if ENABLE_WINNER78_ATTACKER_PLAN
  if (r >= 0 && r < 512) g_win78_chain_region_turn[r] = g_current_turn;
#else
  (void)r;
#endif
}
...
static int win78_chain_region_active(int r) {
#if ENABLE_WINNER78_ATTACKER_PLAN
  if (r < 0 || r >= 512) return 0;
  if (g_win78_chain_region_turn[r] <= 0) return 0;
  return g_current_turn - g_win78_chain_region_turn[r] <= WIN78_CHAIN_REGION_TTL;
#else
  (void)r;
  return 0;
#endif
...
static void win78_clear_chain_region(int r) {
#if ENABLE_WINNER78_ATTACKER_PLAN
  if (r >= 0 && r < 512) g_win78_chain_region_turn[r] = 0;
#else
  (void)r;
#endif
}

static void sanitize_win78_strict_attack_policy(Actions *a,
                                                const GameState *S,
                                                const GameMap *M) {
#if ENABLE_WINNER78_ATTACKER_PLAN
  /* Before the first 1-78-style HQ 9-stack, do not allow legacy attacks to
     leak out.  Expansion/near-neutral capture is still handled by the old
     economy logic, but any move aimed at an enemy building/occupied region is
     dropped unless it is the first launched HQ wave itself. */
  if (!g_win78_first_hq_wave_launched) {
    int wnew = 0;
    for (int i = 0; i < a->moves.len; ++i) {
      const Warrior *w = find_warrior_const(S, a->moves.data[i].id);
      if (w != NULL && w->region == M->my_hq &&
          anchor_rush_region_is_enemy_objective(S, M, a->moves.data[i].target)) {
        /* This can only be the first HQ wave if issue_win78_hq_stack_phase()
           already set the launch flag.  Since the flag is still false here,
           treat it as an early leak and remove it. */
        continue;
      }
      if (anchor_rush_region_is_enemy_objective(S, M, a->moves.data[i].target))
        continue;
  ...
  /* After HQ-9 mode starts, NEW waves are legal only if they are a full
     packet launched from HQ.  However, a packet that has already launched and
     captured/destroyed a base must keep rolling from the conquered region with
     all surviving soldiers until the soldiers die.  Such roll-forward sources
     are tracked in g_win78_chain_region_turn and are exempt from the HQ/9
     origin check. */
  int keep[1024];
  for (int i = 0; i < a->moves.len && i < 1024; ++i) keep[i] = 1;
  if (anchor_rush_enemy_base_count(S, M) > 0) {
    for (int i = 0; i < a->moves.len && i < 1024; ++i) {
      if (a->moves.data[i].target == M->opp_hq) keep[i] = 0;
    }
  }
  for (int i = 0; i < a->moves.len; ++i) {
    if (i >= 1024) break;
    if (!anchor_rush_region_requires_full_packet(S, M, a->moves.data[i].target))
      continue;
    const Warrior *wi = find_warrior_const(S, a->moves.data[i].id);
    int src = wi ? wi->region : -1000000;
    int tgt = a->moves.data[i].target;
    int cnt = 0;
    for (int j = 0; j < a->moves.len; ++j) {
      const Warrior *wj = find_warrior_const(S, a->moves.data[j].id);
      int s2 = wj ? wj->region : -1000001;
      if (s2 == src && a->moves.data[j].target == tgt) ++cnt;
    }
    if (!((src == M->my_hq && cnt >= ANCHOR_RUSH_WAVE) ||
          win78_chain_region_active(src))) {
      for (int j = 0; j < a->moves.len && j < 1024; ++j) {
        const Warrior *wj = find_warrior_const(S, a->moves.data[j].id);
        int s2 = wj ? wj->region : -1000001;
        if (s2 == src && a->moves.data[j].target == tgt) keep[j] = 0;
      }
    }
  }
  int wnew = 0;
  for (int i = 0; i < a->moves.len; ++i)
    if (i >= 1024 || keep[i]) a->moves.data[wnew++] = a->moves.data[i];
  a->moves.len = wnew;

  /* The rush plan should not build newly destroyed enemy bases.  Keep ordinary
     upgrades of existing owned buildings/HQ, but drop fresh BASE claims after
     the 9-stack is underway so the army never gets pinned by the upgrade
     legality guard. */
  {
    int unew = 0;
    for (int i = 0; i < a->upgrades.len; ++i) {
      int r = a->upgrades.data[i];
      if (g_anchor_rush_active && r != M->my_hq &&
          anchor_rush_is_new_capture_upgrade(S, M, r))
        continue;
      a->upgrades.data[unew++] = r;
    }
    a->upgrades.len = unew;
  }
#else
...
static int anchor_rush_pick_target(const GameState *S, const GameMap *M,
  ...
  /* Keep attacking enemy BASEs while any enemy BASE exists.  Also avoid BASE
     targets whose shortest route would pass through the enemy HQ first; that
     looks like an HQ attack in the log and can prematurely damage/destroy HQ. */
  if (anchor_rush_should_hit_hq_now(S, M)) return M->opp_hq;

  int best = -1, bd = 1000000, best_guard = -1;
  for (int i = 0; i < S->buildings.len; ++i) {
    const Building *b = &S->buildings.data[i];
    if (b->side != opposite(M->my_side)) continue;
    if (b->type != BTYPE_BASE) continue;
    if (P->nxt[anchor][b->region] == -1) continue;
    if (anchor_rush_path_hits_region_before_target(P, anchor, b->region,
                                                   M->opp_hq))
      continue;
    int d = path_hops_between(P, anchor, b->region);
    int guard = enemy_warrior_count_at(S, M, b->region);
    if (best < 0 || d < bd || (d == bd && guard > best_guard)) {
      best = b->region;
      bd = d;
      best_guard = guard;
    }
  ...
  /* Enemy BASEs still exist, but every reachable route would go through HQ.
     Obey the user rule and do not start an HQ attack yet. */
  return -1;
}

static int issue_win78_chain_rollforward(Actions *a, const GameState *S,
                                           const GameMap *M, const Paths *P,
                                           int *budget) {
#if ENABLE_WINNER78_ATTACKER_PLAN
  int issued = 0;
  for (int r = 0; r < 512; ++r) {
    if (!win78_chain_region_active(r)) continue;
    if (r == M->my_hq) continue;

    int cnt = 0;
    for (int i = 0; i < S->warriors.len; ++i) {
      const Warrior *w = &S->warriors.data[i];
      if (w->id.side == M->my_side && w->state == WSTATE_STATIONARY &&
          w->region == r && !action_has_move_warrior(a, w->id))
        ++cnt;
    }

    if (cnt <= 0) {
      /* The stack is still travelling or died.  Keep the mark alive for a TTL;
         it may arrive next turn. */
      continue;
    }

    const Building *b = find_building_const(S, r);
    if (b != NULL && b->side == opposite(M->my_side)) {
      /* Still sieging/fighting at the current objective; pin the attackers so
         no other module recalls them. */
      for (int i = 0; i < S->warriors.len; ++i) {
        const Warrior *w = &S->warriors.data[i];
        if (w->id.side == M->my_side && w->state == WSTATE_STATIONARY &&
            w->region == r && !action_has_move_warrior(a, w->id)) {
    ...
    /* If the building is gone, do not stop just because a stray enemy warrior
       remains on the rubble.  The 1-78-style attack keeps tempo by rolling the
       surviving squad toward the next base instead of finishing every local
       skirmish. */

    int target = anchor_rush_pick_target(S, M, P, r);
    ...
    for (int i = 0; i < S->warriors.len; ++i) {
      ...
      if (w->id.side != M->my_side || w->state != WSTATE_STATIONARY ||
          w->region != r) continue;
      if (action_has_move_warrior(a, w->id)) continue;
      /* Roll-forward is deliberately tempo-first: once the HQ 9-stack has
         cracked a base, survivors keep marching as a single packet even if
         enemy remnants or intermediate enemy material are on the path.  The
         normal add_move guard is too conservative here, so issue the legal MOVE
         directly and pay the cost. */
      if (*budget >= MOVE_COST && P->nxt[w->region][target] != -1) {
        Move mv = {w->id, target};
        VEC_PUSH(a->moves, mv);
        *budget -= MOVE_COST;
        ++sent;
    ...
    if (sent > 0) {
      win78_note_chain_region(target);
      /* Keep the old source marked through final sanitization.  Otherwise the
         sanitizer sees a non-HQ, sub-9 roll-forward source after we clear it
         and deletes the very moves we just issued.  The mark expires by TTL. */
      issued += sent;
    }
  }
  return issued;
...
static int issue_anchor_rush(Actions *a, const GameState *S, const GameMap *M,
  ...
  (void)turn;
#if ENABLE_WINNER78_ATTACKER_PLAN
  g_anchor_rush_active = (g_win78_first_hq_wave_launched &&
                          win78_ready_for_anchor_rush(S, M));
#else
  ...
  /* 0) ATTACK SUCCEEDED -> BUILD: our warriors standing on an unbuilt
     ...
     our base if affordable) and send the whole surviving group
     on to the next target; no warrior is reserved as a garrison.  Claims outrank training: if we cannot
  ...
  for (int si = 0; si < M->strongholds.len; ++si) {
    ...
    if (here <= 0) continue;
    /* A destroyed enemy BASE becomes an empty stronghold.  Do NOT build it in
       the winner-1-78 attacker plan; tempo belongs to the army.  Also do not
       send a damaged 1-2 survivor drip.  Only a full 9-stack that survived the
       fight may immediately roll to the next objective. */
    if (here >= ANCHOR_RUSH_WAVE) {
      ...
      for (int i = 0; i < S->warriors.len; ++i) {
        const Warrior *w = &S->warriors.data[i];
        if (w->id.side == M->my_side && w->region == r &&
            w->state == WSTATE_STATIONARY && !action_has_move_warrior(a, w->id)) {
          Move hold = {w->id, r};
          VEC_PUSH(a->moves, hold);
        }
      }
    }
    /* no UPGRADE / no claim after breaking a base */
  }

  /* 0.4) In this strict 1-78 attacker plan we do NOT reserve money to build
     conquered bases.  The wave should keep rolling instead of losing tempo to
     a fresh BASE claim. */
...
#define ANCHOR_RUSH_BASE_LEVEL 1
#endif
  if (!claim_pending) {
    for (int bi = 0; bi < S->buildings.len; ++bi) {
      const Building *b = &S->buildings.data[bi];
  ...
  }
  {
    ...
    for (int i = 0; i < S->warriors.len; ++i) {
      ...
      if (b != NULL && b->side == M->my_side) {
        /* Normal owned bases keep a small garrison, but a freshly captured
           rush base is a roll-forward staging point: leave nobody there and
           send all survivors onward. */
        int keep = (w->region == M->my_hq) ? ANCHOR_RUSH_LEAVE_HOME : 1;
        if (w->region == g_rush_last_claim_region &&
      ...
      }
      {
        ...
        if (target >= 0 && b != NULL && b->side == M->my_side &&
            w->region < 512 &&
            mine_at[w->region] >= ANCHOR_RUSH_WAVE) {
          /* CONCURRENT conquest (user rule): the victorious group at a
             conquered base attacks the NEXT base together IMMEDIATELY - one
             warrior stays to build, the rest press on.  Pick the next target
             FROM THIS NEWLY WON BASE, not from the old anchor, so the wave
             rolls forward instead of walking back or hitting a stale target.

             For ordinary old bases keep the old anti-drip threshold: at least
             two attackers leave after the garrison.  For a just-captured base,
             allow even a single survivor to continue so the
             conquest chain does not stall. */
          int forward_target = anchor_rush_pick_target(S, M, P, w->region);
          if (forward_target >= 0) dest = forward_target;
          else dest = target;
  ...
  /* fresh conquest at the anchor: the >=WAVE check is ONLY for massing fresh
     troops.  A base we just WON skips it - every survivor attacks NOW
     (user rule: leave no one behind on the fresh base).  The phantom holds below must also be
     skipped, or they block the launch (bug found via live-position dump:
     bases 57/84 kept 4/3 idle warriors forever). */
  int fresh_conquest = 0;  /* strict plan: a fresh conquest is NOT allowed to
                            bypass the 9-stack requirement. */
  ...
  /* 3) wave launch: enough at the anchor -> attack nearest enemy building.
     ...
     victorious survivors press to the next base IMMEDIATELY.  Do not leave a
     builder/garrison on the new base; every stationary survivor may move. */
  {
    int leave_for_launch = ANCHOR_ROUTE_LEAVE_AT_ANCHOR;
    int need = ANCHOR_RUSH_WAVE + leave_for_launch;
    if (at_anchor >= need) {
    int target = anchor_rush_pick_target(S, M, P, anchor);
    int sent = 0;
    for (int i = 0; i < S->warriors.len &&
                    sent < at_anchor - leave_for_launch; ++i) {
      const Warrior *w = &S->warriors.data[i];
      if (w->id.side != M->my_side) continue;
      if (w->state != WSTATE_STATIONARY || w->region != anchor) continue;
      if (action_has_move_warrior(a, w->id)) continue;
...
static int win78_choose_hq_wave_target(const GameState *S, const GameMap *M,
                                       const Paths *P, const Actions *a,
                                       int budget) {
  int target = -1;
  /* Prefer the original capture logic's target when it has one; this keeps the
     target/timing taste of the old bot while forcing the 1-78-style 9-stack
     launch from HQ. */
  if (choose_capturable_enemy_base(S, M, P, a, budget, &target) &&
      !anchor_rush_path_hits_region_before_target(P, M->my_hq, target,
                                                  M->opp_hq))
    return target;

  /* Fallback: pick the easiest enemy BASE for a 9-warrior HQ wave. */
  int best = -1;
  ...
  for (int bi = 0; bi < S->buildings.len; ++bi) {
    ...
    if (b->side != opposite(M->my_side) || b->type != BTYPE_BASE) continue;
    if (P->nxt[M->my_hq][b->region] == -1) continue;
    if (anchor_rush_path_hits_region_before_target(P, M->my_hq, b->region,
                                                   M->opp_hq))
      continue;
    if (!attack_path_first_enemy_is_target(S, M, P, M->my_hq, b->region)) continue;
    int defenders = enemy_warrior_count_at(S, M, b->region);
    long long score = 1000000LL * defenders + 100000LL * b->level +
                      1000LL * b->hp + P->dist[M->my_hq][b->region];
    if (score < best_score) {
      best_score = score;
      best = b->region;
    }
  ...
  if (best >= 0) return best;
  if (anchor_rush_enemy_base_count(S, M) <= 0) return M->opp_hq;
  return -1;
}

static int issue_win78_hq_stack_phase(Actions *a, const GameState *S,
                                      const GameMap *M, const Paths *P,
                                      int *budget) {
#if ENABLE_WINNER78_ATTACKER_PLAN
  /* 1-78 attacker policy: fresh waves are launched from HQ only after
     ANCHOR_RUSH_WAVE stationary warriors have gathered there.  Once a 9-stack
     has left HQ, that SAME stack is allowed to roll forward from any base it
     destroys/captures, with all surviving soldiers, until it dies. */
  if (!g_win78_hq9_mode_started) {
    if (!win78_ready_for_anchor_rush(S, M)) return 0;
    g_win78_hq9_mode_started = 1;
  }

  if (g_win78_first_hq_wave_launched) {
    int rolled = issue_win78_chain_rollforward(a, S, M, P, budget);
    if (rolled > 0) return 1;
  }
  ...
  if (at_hq < ANCHOR_RUSH_WAVE) {
    /* Pin the partial stack at HQ so ordinary expansion/refill does not leak it
       out one-by-one.  Then train toward the 9-stack if possible. */
    for (int i = 0; i < S->warriors.len; ++i) {
      const Warrior *w = &S->warriors.data[i];
      if (w->id.side != M->my_side || w->state != WSTATE_STATIONARY ||
          w->region != M->my_hq) continue;
      if (action_has_move_warrior(a, w->id)) continue;
      Move hold = {w->id, M->my_hq};
      VEC_PUSH(a->moves, hold);
    }
    if (a->train_n == 0 && *budget >= TRAIN_COST) {
      int cap = planned_train_cap(S, M, a);
      int need = ANCHOR_RUSH_WAVE - at_hq;
      int n = min_int(cap, min_int(need, *budget / TRAIN_COST));
      if (n > 0) {
        a->train_n = n;
        *budget -= TRAIN_COST * n;
      }
  ...
  int target = win78_choose_hq_wave_target(S, M, P, a, *budget);
  if (target < 0) return 1;
  int sent = 0;
  for (int i = 0; i < S->warriors.len && sent < ANCHOR_RUSH_WAVE; ++i) {
    const Warrior *w = &S->warriors.data[i];
    if (w->id.side != M->my_side || w->state != WSTATE_STATIONARY ||
        w->region != M->my_hq) continue;
    if (action_has_move_warrior(a, w->id)) continue;
  ...
  if (sent >= ANCHOR_RUSH_WAVE) {
    /* Mark the fresh HQ packet as a chain.  Future attacks by this same packet
       will be allowed from the region it conquers, even with fewer than nine
       survivors.  New independent packets still require HQ+9. */
    g_win78_first_hq_wave_launched = 1;
    win78_note_chain_region(target);
    return 1;
  }
  return 1;
...
static Actions anchor_rush_finish(Actions a, const GameState *S,
                                  const GameMap *M) {
#if ENABLE_ANCHOR_RUSH || ENABLE_WINNER78_ATTACKER_PLAN
  int wnew = 0;
  for (int i = 0; i < a.moves.len; ++i) {
    const Warrior *w = find_warrior_const(S, a.moves.data[i].id);
  ...
  sanitize_win78_strict_attack_policy(&a, S, M);
...
static Actions decide(const GameState *S, const GameMap *M, const Paths *P,
...
#if ENABLE_WINNER78_ATTACKER_PLAN
  /* A launched HQ-9 stack that has just cracked a base keeps tempo above all
     ordinary base-defense/economy moves.  Otherwise a harmless recall can spend
     the last 10 gold and make the rolling squad leave one survivor behind. */
  if (g_win78_first_hq_wave_launched) {
    int rolled = issue_win78_chain_rollforward(&a, S, M, P, &budget);
    if (rolled > 0) {
      update_previous_snapshot(S);
  ...
  /* 1-78-style attacker waves: after map-size base gate and HQ2, every
     attack must be launched from HQ as a full 9-warrior packet.  While this
     phase is active we return immediately, so the legacy/anchor roll-forward
     attackers cannot leak 1-2 unit or non-HQ attacks. */
  if (issue_win78_hq_stack_phase(&a, S, M, P, &budget)) {
    sanitize_anchor_route_offensive_moves(&a, S, M);
    update_previous_snapshot(S);
    return anchor_rush_finish(a, S, M);
  }

  /* Fallback attacker-mode rush offense.  With ENABLE_WINNER78_ATTACKER_PLAN=1
     the HQ-9 phase above normally consumes the turn after the gate opens; this
     remains only for non-winner78 builds. */
  ...
  /* Winner-1-78 attacker phase gate: after the required number of BASEs is
     online, lock spending to HQ level 2 before the 9-stack anchor attack is
     allowed to start. */
  if (!defense_plan.triggered && issue_win78_hq2_priority(&a, S, M, P, &budget)) {
    sanitize_anchor_route_offensive_moves(&a, S, M);
    update_previous_snapshot(S);
    return anchor_rush_finish(a, S, M);
  }
